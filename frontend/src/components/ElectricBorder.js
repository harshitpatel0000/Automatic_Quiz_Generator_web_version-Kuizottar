import React, { useRef, useEffect, useMemo, useId } from 'react';

// Helper function
const hexToRgba = (hex, alpha = 1) => {
    if (!hex) return `rgba(0,0,0,${alpha})`;
    let h = hex.replace('#', '');
    if (h.length === 3) {
        h = h.split('').map(c => c + c).join('');
    }
    const int = parseInt(h, 16);
    const r = (int >> 16) & 255;
    const g = (int >> 8) & 255;
    const b = int & 255;
    return `rgba(${r}, ${g}, ${b}, ${alpha})`;
};

export default function ElectricBorder({
    color = '#5227FF',
    speed = 1,
    chaos = 1,
    thickness = 2,
    className = '',
    children,
    ...props
}) {
    const rootRef = useRef(null);
    const svgRef = useRef(null);
    const strokeRef = useRef(null);
    
    // Generate a unique ID for the filter
    const uniqueId = useId();
    const filterId = `turbulent-displace-${uniqueId}`;

    // This effect updates the animation parameters
    useEffect(() => {
        const svg = svgRef.current;
        const host = rootRef.current;
        if (!svg || !host) return;

        if (strokeRef.current) {
            strokeRef.current.style.filter = `url(#${filterId})`;
        }

        const width = Math.max(1, Math.round(host.clientWidth || 0));
        const height = Math.max(1, Math.round(host.clientHeight || 0));

        const dyAnims = Array.from(svg.querySelectorAll('feOffset > animate[attributeName="dy"]'));
        if (dyAnims.length >= 2) {
            dyAnims[0].setAttribute('values', `${height}; 0`);
            dyAnims[1].setAttribute('values', `0; -${height}`);
        }

        const dxAnims = Array.from(svg.querySelectorAll('feOffset > animate[attributeName="dx"]'));
        if (dxAnims.length >= 2) {
            dxAnims[0].setAttribute('values', `${width}; 0`);
            dxAnims[1].setAttribute('values', `0; -${width}`);
        }

        const baseDur = 6;
        const dur = Math.max(0.001, baseDur / (speed || 1));
        [...dyAnims, ...dxAnims].forEach(a => a.setAttribute('dur', `${dur}s`));

        const disp = svg.querySelector('feDisplacementMap');
        if (disp) disp.setAttribute('scale', String(30 * (chaos || 1)));
        
        // Restart animations
        [...dyAnims, ...dxAnims].forEach(a => {
            if (typeof a.beginElement === 'function') {
                try { a.beginElement(); } catch (e) {}
            }
        });

    }, [speed, chaos, filterId]);

    // This effect handles the resize observer
    useEffect(() => {
        const host = rootRef.current;
        if (!host) return;

        const update = () => { /* We trigger updates via the other effect */ };
        
        const resizeObserver = new ResizeObserver(update);
        resizeObserver.observe(host);

        return () => {
            resizeObserver.disconnect();
        };
    }, []);

    // --- Computed Styles (using useMemo for optimization) ---
    
    // We can't easily get the computed borderRadius, so we'll pass it via className
    // The parent component should have `rounded-full`, etc.
    const inheritRadius = {}; 

    const strokeStyle = useMemo(() => ({
        ...inheritRadius,
        borderWidth: `${thickness}px`,
        borderStyle: 'solid',
        borderColor: color
    }), [thickness, color]);

    const glow1Style = useMemo(() => ({
        ...inheritRadius,
        borderWidth: `${thickness}px`,
        borderStyle: 'solid',
        borderColor: hexToRgba(color, 0.6),
        filter: `blur(${0.5 + thickness * 0.25}px)`,
        opacity: 0.5
    }), [thickness, color]);

    const glow2Style = useMemo(() => ({
        ...inheritRadius,
        borderWidth: `${thickness}px`,
        borderStyle: 'solid',
        borderColor: color,
        filter: `blur(${2 + thickness * 0.5}px)`,
        opacity: 0.5
    }), [thickness, color]);

    const bgGlowStyle = useMemo(() => ({
        ...inheritRadius,
        transform: 'scale(1.08)',
        filter: 'blur(32px)',
        opacity: 0.3,
        zIndex: -1,
        background: `linear-gradient(-30deg, ${hexToRgba(color, 0.8)}, transparent, ${color})`
    }), [color]);

    return (
        <div ref={rootRef} className={`relative isolate ${className}`} {...props}>
            <svg
                ref={svgRef}
                className="fixed -left-[10000px] -top-[10000px] w-[10px] h-[10px] opacity-[0.001] pointer-events-none"
                aria-hidden="true"
                focusable="false"
            >
                <defs>
                    <filter id={filterId} colorInterpolationFilters="sRGB" x="-200%" y="-200%" width="500%" height="500%">
                        <feTurbulence type="turbulence" baseFrequency="0.02" numOctaves="10" result="noise1" seed="1" />
                        <feOffset in="noise1" dx="0" dy="0" result="offsetNoise1">
                            <animate attributeName="dy" values="700; 0" dur="6s" repeatCount="indefinite" calcMode="linear" />
                        </feOffset>
                        <feTurbulence type="turbulence" baseFrequency="0.02" numOctaves="10" result="noise2" seed="1" />
                        <feOffset in="noise2" dx="0" dy="0" result="offsetNoise2">
                            <animate attributeName="dy" values="0; -700" dur="6s" repeatCount="indefinite" calcMode="linear" />
                        </feOffset>
                        <feTurbulence type="turbulence" baseFrequency="0.02" numOctaves="10" result="noise1" seed="2" />
                        <feOffset in="noise1" dx="0" dy="0" result="offsetNoise3">
                            <animate attributeName="dx" values="490; 0" dur="6s" repeatCount="indefinite" calcMode="linear" />
                        </feOffset>
                        <feTurbulence type="turbulence" baseFrequency="0.02" numOctaves="10" result="noise2" seed="2" />
                        <feOffset in="noise2" dx="0" dy="0" result="offsetNoise4">
                            <animate attributeName="dx" values="0; -490" dur="6s" repeatCount="indefinite" calcMode="linear" />
                        </feOffset>
                        <feComposite in="offsetNoise1" in2="offsetNoise2" result="part1" />
                        <feComposite in="offsetNoise3" in2="offsetNoise4" result="part2" />
                        <feBlend in="part1" in2="part2" mode="color-dodge" result="combinedNoise" />
                        <feDisplacementMap
                            in="SourceGraphic"
                            in2="combinedNoise"
                            scale="30"
                            xChannelSelector="R"
                            yChannelSelector="B"
                        />
                    </filter>
                </defs>
            </svg>
            
            <div className="absolute inset-0 pointer-events-none" style={inheritRadius}>
                <div ref={strokeRef} className="absolute inset-0 box-border" style={strokeStyle}></div>
                <div className="absolute inset-0 box-border" style={glow1Style}></div>
                <div className="absolute inset-0 box-border" style={glow2Style}></div>
                <div className="absolute inset-0" style={bgGlowStyle}></div>
            </div>
            
            <div className="relative" style={inheritRadius}>
                {children}
            </div>
        </div>
    );
}